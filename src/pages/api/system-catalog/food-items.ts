...
export function createGetFoodItemsSystemCatalogRoute(
  resolveApplication: typeof createLemonWebApplication = createLemonWebApplication,
): APIRoute {
  return async ({ request }) => {
    const application = resolveApplication();
    const catalog = await application.getSystemCatalog.execute("food-items");
    if (!catalog) {
...
export const GET = createGetFoodItemsSystemCatalogRoute();
